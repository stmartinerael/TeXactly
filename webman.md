...
that the German word for ``weave'' is ``*web\/*'', and the
...
here. (1)~Myrtle Kellington, as executive editor for ACM
...
editors. (2)~Bill McKeeman introduced a program intended to automate
some of this task [Algorithm 268, ``Algol~60 reference language editor,''
*CACM \bf8* (1965), 667--668]; and a considerable flowering of such
programs has occurred in recent years [see especially Derek Oppen,
``Prettyprinting,'' *ACM TOPLAS \bf2* (1980), 465--483; G.~A. Rose and
J. Welsh, ``Formatted programming languages,'' *SOFTWARE Practice
\char`\&\ Exper.\ \bf11* (1981), 651--669]. (3)~The top-down style of
...
being formulated. (4)~Edwin Towster has proposed a similar style of
...
data,'' *IEEE Trans.\ on Software Eng.\ \bf SE--5* (1979), 374--386];
...
module names on psychological grounds. (5)~Discussions with Luis
Trabb~Pardo in the spring of 1979 were extremely helpful for setting up a
prototype version of `WEB` that was called `DOC`. (6)~Ignacio
...
its present form. (7)~David~R. Fuchs made several crucial suggestions
...
a wide variety of Pascal\ compilers. (8)~The name `WEB` itself
...
A `WEB` file is built up from units called *modules\/* that are more
...
\yskip The three parts of each module must appear in this order;
...
to be *unnamed*. Note that module names cannot appear in the
...
\yskip\hang``@d` \{identifier} \.= \{constant}' defines a *numeric\/*
macro, allowing `TANGLE` to do rudimentary arithmetic.

\yskip\hang``@d` \{identifier} `==` Pascal\ text' defines a *simple\/* macro, where the identifier will be replaced by the Pascal\ text
when `TANGLE` produces its output.

\yskip\hang``@d` \{identifier}`(\#) ==` Pascal\ text' defines a
*parametric\/* macro, where the identifier will be replaced by the Pascal\
text and where occurrences of `\#` in that Pascal\ text will be
replaced by an argument.

\yskip In all three cases, the identifier must have length greater
than one; it must not be a single letter.

Numeric macros are subject to the following restrictions: 
(1)~The identifier must
be making its first appearance in the `WEB` file;
a numeric macro must be defined before it is used. 
...
numeric macro. A *preprocessed string\/* is a string that is like
...
converted into an integer equal to 256 or more. A *string pool\/*
...
called the *string pool check sum*. If any string changes, the check
...
these special features. A `WEB` *control code\/*
...
in Appendix C of *The TeX book\/* can be used in strings. But you should
...
left brace *does\/* count.) At present, `TANGLE` and `WEAVE` treat
...
#4{\yskip`#1` applied to `#2` (cpu time #3 sec)\par
\halign{ \tt##\hfil\cr#4}}
...
\yskip The cpu time for Pascal\ to process `TANGLE.PAS` was
...
{\baselineskip9pt
\halign to{\hfil\quoteit#\tabskip 0pt plus 100pt&
...
\xdef\fmtversion{\fmtversion+WEBMAC4.0} 
...
\newcount\ind 
 ...
 \hangindent\ind em\kern\ind em\copy\bakk\fi}

 \XX\hfil ${\,$#1$\,}$\XX}

 

 \def\rhead{\uppercase{#2}} 
 \message{*\modno} 
 \edef\next{\write\cont{\Z{#2}{\modno}{\the 
 \ifon\startsection{\bf#2. }}
...
\newif\ifon \newif\iftitle \newif\ifpagesaved
 ...
 \else\hbox to\pagewidth{\vbox to10pt{}\ifodd #3\else#2\fi}\fi
...
\newdimen\fullpageheight \fullpageheight=9in 
...
\newbox\lbox 
 ...
 \parfillskip 0pt plus .6 
 \def\lr{L} 
 \output{\if L\lr\global\setbox\lbox=\page \gdef\lr{R}
 \else\normaloutput{\vbox to\pageheight{\box\sbox\vss
 \hbox to\pagewidth{\box\lbox\hfil\page}}}\lheader\rheader
 \global{L}\global\pagesavedfalse\fi}
 \message{Index:}
 \parskip 0pt plus .5pt
 \outer\def\:##1, {\par\hangindent2em##1:\kern1em} 
 \let\ttentry=\. \def\.##1{\ttentry{##1\kern.2em}} 
 \def\[##1]{$\underline{##1}$} 
...
`PLAIN` format: You can say ``{\\sc STUFF\`}' to get *STUFF*
...
$$\lpile{`\\def\\topofcontents{\\null\
 ...
 ` { `\\def\\rheader{\ Appendix D\\hfil 15}}\cr
 ` { `\\centerline{\\titlefont The {\\ttitlefont WEAVE}{ }processor}}\cr
 ` { `\ 15pt \\centerline{(Version 4)}{ }\
